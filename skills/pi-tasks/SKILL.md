...
4. `TaskRun` ready tasks through `pi-subagents`.
5. `TaskStatus`/`TaskOutput`/`TaskWait` to inspect or refresh run state.
6. `TaskResume` or `TaskRetry` only when the run lifecycle calls for it.
7. `TaskUpdate` or follow-up tasks for unresolved work.
